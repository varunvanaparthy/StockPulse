...
class EstimateScreen(Screen) :
    def on_enter(self,*args) :
        def stockValue(day,c0,c1,c2,c3) :
            return c0 + c1*day - c2*np.exp(-c3*day)
        ...
        guess = [100,0.2,150,-0.02] 
        c,cov = curve_fit(stockValue,X_train,Y_train)
        Y_est = []
        for i in X_test : 
            Y_est.append(stockValue(i,c[0],c[1],c[2],c[3]))
        #print(c)
        plt.plot(X_test,Y_est,color='c',linewidth=2.0)
        plt.scatter(X_test,Y_est,color='r',s=20)
        plt.title("Estimate data of " + self.manager.get_screen('menu').selct_comp + " for the next " + str(len(X_test)) +  " days : ")
